      ...
      (sm.chips || []).forEach(function (c) {
        ...
        if (c.prop === 51 || c.prop === 211) {
          ...
          sp.buffs.push({ p: c.prop, name: GA.statName(c.prop), v: c.value, pct: c.isPct,
                   ...
                   && GA.statName(c.prop)) {
          // riders the payload lands with every hit - the Lockdown beam's -30% slow, the
          // Venom mine's shred. Re-applied per volley to whoever the payload is hitting.
          sp.debuffs.push({ p: c.prop, name: GA.statName(c.prop), v: -c.value, pct: c.isPct,
                            cat: c.cat, app: c.app || 0, appv: c.appv || 0, life: lifeOf(c) });
        }
      });
    });
    if (d.spawn && !d.spawn.shots.length && !d.spawn.dots.length && !d.spawn.heals.length
        && !d.spawn.powers.length && !d.spawn.buffs.length && !d.spawn.debuffs.length
        && !(d.spawn.strip || []).length && !d.spawn.hp) d.spawn = null;
    // Deploy time and lifespan live on the CARRIER's stat chips (a turret takes 25s to
    // build itself, a station 15; a drone flies for 10s - skill-extended through resolve),
    // not on the payload's own rows. A turret placed mid-fight is a promise, not a gun,
    // and a drone is a visitor, not a fixture. Welding acceleration is not yet modelled;
    // the resolved deploy figure is the unwelded one.
    if (d.spawn && d.deployTime > 0) {
      d.spawn.deploy = Math.max(d.spawn.deploy, d.deployTime);
    }
    if (d.spawn && d.lifespan > 0 && !d.spawn.life) {
      d.spawn.life = d.lifespan;
    }
      ...
      S.actors.forEach(function (a) {
        ...
        a.spawns.forEach(function (ins) {
          ...
          dmgTargets.forEach(function (tid2) {
            ...
            // riders the payload lands with the hit - the Lockdown beam's slow refreshes
            // for as long as the beam holds; protection can refuse them outright (S31)
            (sp.debuffs || []).forEach(function (df) {
              var dfLife = effectLifeAfterProt(v, df.cat, df.life, sp.hit.rating);
              if (!(dfLife > 0)) return;
              v.live = v.live.filter(function (x) {
                return !(x.src === sp.name && x.p === df.p);
              });
              v.live.push({ p: df.p, name: df.name, v: df.v, pct: df.pct, cat: df.cat,
                            app: df.app, appv: df.appv, at: t, aura: 1,
                            src: sp.name, until: t + dfLife, devId: ins.devId });
            });
          });
